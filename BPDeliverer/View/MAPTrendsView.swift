...
extension MAPTrendsReducer.State {
        ...
        WithViewStore(store, observe: {$0}) { viewStore in
            WithViewStore(store, observe: {$0}) { viewStore in
                ScrollView{
                    VStack(spacing: 35){
                        VStack(spacing: 16){
                            NavigationBarView(backAction: {viewStore.send(.showBackAD)}, title: "MAP Trends")
        ...
        var body: some View {
            WithViewStore(store, observe: {$0}) { viewStore in
                VStack(spacing: 16){
                    ...
                    Text(LocalizedStringKey("The normal range 79-110 mmHg")).font(.system(size: 12.0)).foregroundStyle(Color("#BBCDD9"))
                }
            }
        }
    }
